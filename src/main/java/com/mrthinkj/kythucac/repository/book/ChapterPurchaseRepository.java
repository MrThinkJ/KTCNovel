...
public interface ChapterPurchaseRepository extends CrudRepository<ChapterPurchase, Integer> {
    ...
    List<Map<Integer, LocalDate>> findRevenueLastTenDaysByBook(int bookId);
    @Query(value = "select chapter_name, purchase_date, chapter_price, user_name, u.id from chapter_purchase as cp join chapter c on c.id = cp.chapter_id join account a on a.id = cp.account_id join user u on u.id = a.id where c.book_id = ?1 order by purchase_date desc limit 20;", nativeQuery = true)
    List<Object[]> findLast20ChapterPurchase(int bookId);

    default List<ChapterPurchaseDTO> findLast20ChapterPurchaseConvert(int bookId){
        List<Object[]> result = findLast20ChapterPurchase(bookId);
